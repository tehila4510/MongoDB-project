...
### הרצה בלי להתקין MongoDB (הכי מהיר)

יש כאן גם כלי שמריץ הכל אוטומטית מול מסד נתונים זמני (in-memory) — צריך רק Node.js:
...
זה מקים MongoDB זמני, מריץ את שני הקבצים ומדפיס את התוצאות, ואז סוגר הכל. (מומש בקובץ `run-with-mongo.js`.)
